...
export default function ContactModal({
  ...
  // Liste des rattachements : [{ type: "client"|"subcontractor", entiteId: string }]
  ...
  useEffect(() => {
    if (interlocuteur) {
      ...
      // Compatibilité ancien format (entiteId unique) et nouveau format (entites[])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (rattachements.length === 0) {
      setError("Veuillez sélectionner au moins un client ou sous-traitant.");
      return;
    }
    try {
      setLoading(true);
      // On conserve type/entiteId pour compatibilité (premier rattachement)
      // + entites[] pour le nouveau multi-rattachement
      await onSave({
        nom: nom.trim(),
        prenom: prenom.trim(),
        email: email.trim(),
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-2">
              Clients rattachés
              <span className="text-gray-400 font-normal ml-1">(cochez tous les clients concernés)</span>
            </label>
            <div className="border border-slate-200 rounded-lg divide-y divide-slate-100 max-h-40 overflow-y-auto">
              {clients.length === 0 ? (
                <p className="text-xs text-slate-400 italic p-3">Aucun client disponible.</p>
              ) : clients.map(c => (
                <label key={c.id} className="flex items-center gap-2.5 px-3 py-2 hover:bg-teal-50/40 cursor-pointer">
                  <input type="checkbox"
                    checked={isChecked("client", c.id)}
                    onChange={() => toggleRattachement("client", c.id)}
          ...
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-2">
              Sous-traitants rattachés
              <span className="text-gray-400 font-normal ml-1">(cochez tous les sous-traitants concernés)</span>
            </label>
            <div className="border border-slate-200 rounded-lg divide-y divide-slate-100 max-h-40 overflow-y-auto">
              {subcontractors.length === 0 ? (
                <p className="text-xs text-slate-400 italic p-3">Aucun sous-traitant disponible.</p>
              ) : subcontractors.map(s => (
                <label key={s.id} className="flex items-center gap-2.5 px-3 py-2 hover:bg-indigo-50/40 cursor-pointer">
                  <input type="checkbox"
                    checked={isChecked("subcontractor", s.id)}
                    onChange={() => toggleRattachement("subcontractor", s.id)}
          ...
          {rattachements.length > 0 && (
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
              ...
              <div className="flex flex-wrap gap-1.5">
                {rattachements.map(r => {
                  const entite = r.type === "client"
                    ? clients.find(c => c.id === r.entiteId)?.nom
                    : subcontractors.find(s => s.id === r.entiteId)?.nom;
                  return (
                    <span key={`${r.type}-${r.entiteId}`}
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${r.type === "client" ? "bg-teal-100 text-teal-800" : "bg-indigo-100 text-indigo-800"}`}>
                      {entite}
                    </span>
                  );
                })}
